...
class UsersController extends Controller
{
    ...
    public function create_post() {
        ...
        return ['tags' => UsersTagsResource::collection($tags), 'categories' => UsersCategoriesResource::collection(UsersPostsResource::collection($categories))];
    }
    ...
    public function edit_post($post)
    {
        ...
        $tags = Tag::all();
        $categories = Category::whereStatus(1)->get();
        return ['post' =>new UsersPostsResource($post),'tags' => UsersTagsResource::collection($tags), 'categories' => UsersCategoriesResource::collection(UsersPostsResource::collection($categories))];
    }
    ...
    public function delete_post($post_id)
    {
        ...
        return response()->json(['errors' => false,'message' => 'Something was wrong',], 200);
    }

    public function delete_post_media($media)
    {
        $media = PostMedia::whereId($media)->first();
        if ($media) {
            if (File::exists('assets/posts/' . $media->file_name)) {
                unlink('assets/posts/' . $media->file_name);
            }
